...
public class Grasp {
    ...
    public void setEnviosPorDiaPropio() {
        // Aqui lo que se hace es separar los pedidos de acuerdo al día en que aparecen (fecha en la que se realiza el pedido)
        // Finalmente, lo que se va a obtener es un mapa, donde la llave es cada fecha de aparicion
        ...
        // Aqui lo que se realiza es la obtencion de todas las fechas en donde hayan aparecido pedidos, se ordenan ascendentemente
        // de acuerdo a la fecha, y finalmente se utiliza para poder imprimir la cantidad de dias a planificar
    ...
    private ArrayList<VueloInstanciado> instanciarVuelosDiarios(ArrayList<PlanDeVuelo> planesDeVuelo, ZonedDateTime inicio, ZonedDateTime fin) {
        ArrayList<VueloInstanciado> vuelos = new ArrayList<>();

        Long diasTotales = ChronoUnit.DAYS.between(inicio, fin) + 1;
        int diasInstanciar = (int)Math.min(5, diasTotales);
        ...
        for(PlanDeVuelo plan : planesDeVuelo) {
            for(int i = 0; i < diasInstanciar; i++) {
                ...
                ZonedDateTime salida = fecha.atZone(zoneOrigen);
                ZonedDateTime llegada = fecha.atZone(zoneOrigen);

                // Si la hora de llegada es antes que la de salida, significa que ha pasado una noche, por lo que se suma un dia
                if(!llegada.isAfter(salida))
                    llegada = llegada.plusDays(1);

                ZonedDateTime llegadaDestino = llegada.withZoneSameInstant(zoneDestino);
                vuelos.add(new VueloInstanciado(plan, salida, llegadaDestino));  // Se agrega el vuelo a la lista de vuelos instanciados
            }
        }
    ...
    private Solucion ejecutarGrasp(List<Envio> enviosDelDia, RutasDiarias rutasDiarias) {
        ...
        for(int i = 0; i < 1500; i++) {
            ...
            enviosDelDia.forEach(e -> e.getParteAsignadas().clear());  // Se elimina cualquier asignacion que tenga un envio
    ...
    private Boolean esMejor(Solucion a, Solucion b) {
        ...
        // MediaPonderada(a) - MediaPonderada = -Negativo  <- Significa que la MediaPonderada(a) es menor a la de b
    ...
    private void faseConstruccion(List<Envio> enviosDelDia, RutasDiarias rutasDiarias) {
        ...
        for(Envio envio : enviosCopia) {
            ...
            while(envio.cantidadRestante() > 0 && partesUsadas < 3) {
                ...
                if(rutaCandidata.isEmpty()) break;
                ...
                List<CandidatoRuta> rcl = rutaCandidata.stream().filter(c -> c.getScore() <= umbral).collect(Collectors.toList());
                ...
                for(VueloInstanciado v : escogido.getTramos()) {
                    // Por cada vuelo de la ruta candidata elegida, se va a identificar la minima capacidad de los vuelos
                ...
                // Crear la parte asignada y vincularla al envio para mantener la relación bidireccional
                ParteAsignada parte = new ParteAsignada(escogido.getTramos(), escogido.getLlegada(), cant, escogido.getOrigen());
    ...
    private void busquedaLocal(List<Envio> enviosDelDia, RutasDiarias rutasDiarias) {
        ...
        List<Envio> enviosConPartes = envios.stream().filter(e -> !e.getParteAsignadas().isEmpty()).collect(Collectors.toList());
        ...
        for(Envio envio : enviosConPartes) {
            ...
            for(ParteAsignada parte : snapshot) {
                ...
                List<CandidatoRuta> candidato = rutasDiarias.getCandidatosRuta(envio).stream()
                        .filter(c -> {
                            for(VueloInstanciado v : c.getTramos())  // Se verifica que todos los vuelos de la ruta tengan espacio para la cantidad de esta parte
                ...
                if(!candidato.isEmpty()) {  // Hay rutas candidatas
                    ...
                    ParteAsignada nuevaParte = new ParteAsignada(c.getTramos(), c.getLlegada(), parte.getCantidad(), c.getOrigen()); // Se agrega una nueva parte
                ...
                if(!mejorado) {  // Si no mejoro, se restablece la ruta original
                    for(VueloInstanciado v : parte.getRuta())
                        v.asignar(parte.getCantidad());
