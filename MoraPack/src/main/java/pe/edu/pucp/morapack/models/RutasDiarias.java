...
public class RutasDiarias {
    ...
    public RutasDiarias(LocalDateTime dia, ArrayList<PlanDeVuelo> vuelos, ArrayList<Aeropuerto> hubs, ArrayList<Aeropuerto> aeropuertos) {
        this.dia = dia;
        this.vuelos = vuelos;
        this.hubs = hubs;
        ...
        this.vuelosPorOrigenCache = vuelos.stream()
                .collect(
                        Collectors.groupingBy(v -> aeropuertoById.get(v.getCiudadOrigen()).getCodigo()));
        this.vuelosPorOrigenYFecha = vuelos.stream()
                .collect(Collectors.groupingBy(v -> aeropuertoById.get(v.getCiudadOrigen()).getCodigo()
        ...
        for(Aeropuerto origen : envio.getAeropuertosOrigen()) {
            ...
            for(int nivel = 0; nivel < 4; nivel++) {
                ...
                for(PathState ps : beam) {  // Iteramos en cada estado
                    ...
                    List<PlanDeVuelo> salidas = this.vuelosPorOrigenCache.getOrDefault(origen.getCodigo(), Collections.emptyList());
                    ...
                    for(PlanDeVuelo v : salidas) {
                        ...
                        PathState nuevo = new PathState(getAeropuertoById(v.getCiudadDestino()), v.getZonedHoraDestino(), ruta, v, capRuta);

                        // Verificar si llegamos al destino
                        if(getAeropuertoById(v.getCiudadDestino()).getCodigo().equals(envio.getAeropuertoDestino().getCodigo())) {
