...
public class GraspController {
    ...
    public Solucion iniciarGrasp() {
        ...
        System.out.println("Estoy aqui, con el grasp iniciado correctamente");
...
/*
    ...
    public ArrayList<Envio> ejecutarGrasp(@PathVariable String fechaHora) {
        ...
        if(esPrimeraSimulacion) {
            planesEnRango = planDeVueloService.obtenerPlanesDeVueloPorFecha(fechaInicioLocal, husoHorarioStr, fechaFin.plusHours(17).toLocalDateTime());
            grasp.setPlanes(planesEnRango);
            esPrimeraSimulacion = false;
            ultimaFechaConsulta = fechaFin.plusHours(15);
        } else {
            grasp.getPlanes().removeIf(plan -> plan.getZonedHoraOrigen().isBefore(fechaInicio));
            planesEnRango = planDeVueloService.obtenerPlanesDeVueloPorFecha(ultimaFechaConsulta.toLocalDateTime(), husoHorarioStr, ultimaFechaConsulta.plusHours(2).toLocalDateTime());
        ...
        ArrayList<Envio> solucion = grasp.ejecutarGrasp(grasp.getAeropuertos(), grasp.getEnvios(), grasp.getPlanes());
    ...
    public String iniciarGraspDiaria(@PathVariable String fechaHora) {
        ...
        planesEnRango = planDeVueloService.obtenerPlanesDeVueloPorFecha(fechaInicioLocal, husoHorarioDiaria, horaInicioDiaria.plusHours(17).toLocalDateTime());
        grasp.setPlanes(planesEnRango);
    ...
    public ArrayList<Envio> ejecutarGraspDiaria(@PathVariable String fechaHora) {
        ...
        for(Envio envio : enviosEnRango)
            if(!enviosConRutaIds.contains(envio.getId()) && !graspEnviosIds.contains(envio.getId()))
